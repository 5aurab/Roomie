...
class HouseholdSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Household
        fields = [
            'id', 'name', 'address', 'no_of_rooms', 
            'move_in_date', 'join_code', 'created_by', 
            'created_at', 'members_count'
        ]
        read_only_fields = ['join_code', 'created_by', 'created_at', 'members_count']
...
class JoinHouseholdSerializer(serializers.Serializer):
    ...
    def validate_join_code(self, value):
        if not Household.objects.filter(join_code=value.upper()).exists():
...
class HouseholdMemberSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'user', 'user_display_name', 'user_email', 'move_in_date', 'joined_at']
